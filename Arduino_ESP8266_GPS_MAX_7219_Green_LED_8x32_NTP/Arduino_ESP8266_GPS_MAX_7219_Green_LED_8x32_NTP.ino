...
// NTP настройки
WiFiUDP ntpUDP;
NTPClient timeClient(ntpUDP, "pool.ntp.org", 0, 10); // UTC offset 0, обновление каждые 60 сек
...
// Инициализация NTP
  timeClient.begin();
  timeClient.setTimeOffset(7200); // Установите ваш часовой пояс в секундах (здесь +3 UTC для Москвы)
  timeClient.update();
...
//Бесконечный цикл    
    while (true) 
    {
          ...
          if (jj++ > 3000)
          {
            ...
            DateTimeStruct currentTime = getCurrentTime();
...
DateTimeStruct getCurrentTime() 
{
  DateTimeStruct dt;
  
  unsigned long epochTime = timeClient.getEpochTime();
  dt.hours = timeClient.getHours();
  dt.minutes = timeClient.getMinutes();
  dt.seconds = timeClient.getSeconds();

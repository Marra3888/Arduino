...
float GyverBME280::readHumidity(void) {
    Wirer.beginTransmission(_i2c_address);									// Start I2C transmission
    Wirer.write(0xFD);												   		// Request humidity data register	
    if (Wirer.endTransmission() != 0) return 0;										
    Wirer.requestFrom(_i2c_address, 2);										// Request humidity data 
    int32_t hum_raw = ((uint16_t)Wirer.read() << 8) | (uint16_t)Wirer.read();	// Read humidity data 
...
uint32_t GyverBME280::readRegister24(uint8_t address) {	 
    Wirer.beginTransmission(_i2c_address);
    Wirer.write(address);
    if (Wirer.endTransmission() != 0) return 0x800000;
    Wirer.requestFrom(_i2c_address, 3);
    return (((uint32_t)Wirer.read() << 16) | ((uint32_t)Wirer.read() << 8) | (uint32_t)Wirer.read());
}
...
bool GyverBME280::writeRegister(uint8_t address , uint8_t data) {
    ...
    Wirer.write(address);
    Wirer.write(data);
    if (Wirer.endTransmission() != 0) return false;
    return true;
}
...
uint8_t GyverBME280::readRegister(uint8_t address) {
    ...
    Wirer.write(address);
    if (Wirer.endTransmission() != 0) return 0;
    Wirer.requestFrom(_i2c_address , 1);
    return Wirer.read();
}
...
void GyverBME280::readCalibrationData(void) {
    ...
    Wirer.beginTransmission(_i2c_address);
    Wirer.write(0x88);
    if (Wirer.endTransmission() != 0) return;
    Wirer.requestFrom(_i2c_address , 25);
    /* reading */
    CalibrationData._T1 = (Wirer.read() | (Wirer.read() << 8));
    CalibrationData._T2 = (Wirer.read() | (Wirer.read() << 8));
    CalibrationData._T3 = (Wirer.read() | (Wirer.read() << 8));
    CalibrationData._P1 = (Wirer.read() | (Wirer.read() << 8));
    CalibrationData._P2 = (Wirer.read() | (Wirer.read() << 8));
    CalibrationData._P3 = (Wirer.read() | (Wirer.read() << 8));
    CalibrationData._P4 = (Wirer.read() | (Wirer.read() << 8));
    CalibrationData._P5 = (Wirer.read() | (Wirer.read() << 8));
    CalibrationData._P6 = (Wirer.read() | (Wirer.read() << 8));
    CalibrationData._P7 = (Wirer.read() | (Wirer.read() << 8));
    CalibrationData._P8 = (Wirer.read() | (Wirer.read() << 8));
    CalibrationData._P9 = (Wirer.read() | (Wirer.read() << 8));
    CalibrationData._H1 = Wirer.read();

    /* second part request*/
    Wirer.beginTransmission(_i2c_address);
    Wirer.write(0xE1);
    Wirer.endTransmission();
    Wirer.requestFrom(_i2c_address , 8);
    /* reading */
    CalibrationData._H2 = (Wirer.read() | (Wirer.read() << 8));
    CalibrationData._H3 = Wirer.read();
    CalibrationData._H4 = (Wirer.read() << 4);
    uint8_t interVal = Wirer.read();
    CalibrationData._H4 |= (interVal & 0xF);
    CalibrationData._H5 = (((interVal & 0xF0) >> 4) | (Wirer.read() << 4));
    CalibrationData._H6 = Wirer.read();
}
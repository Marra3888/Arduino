// Наикрасивейшие часы Матроскина (спасибо за идею!!!) с моими доработками.
// Управление колбой напрямую, без промежуточных "мозгов"
// Все изменения режимов и настроек сохраняются в EEPROM
// devICEpro 2022г.
...
//EncButton<EB_TICK, 16> butt2;
// EncButton<EB_TICK, 15> butt3;
// EncButton<EB_TICK, VIRT_BTN> vbutt0;  //menu
// EncButton<EB_TICK, VIRT_BTN> vbutt1;  //+
// EncButton<EB_TICK, VIRT_BTN> vbutt2;  //-
// Если используется внешняя подтяжка - лучше перевести в INPUT
//EncButton<EB_TICK, 6, 7, 17> enc(INPUT);
//EncButton<EB_TICK, 16> butt2(INPUT);
//EncButton<EB_TICK, 15> butt3(INPUT);

// #include "AnalogKey.h"

// создаём массив значений сигналов с кнопок
// int16_t sigs[3] = {330, 18, 475};

// указываем пин, количество кнопок и массив значений
// AnalogKey<A2, 3, sigs> keys;

// AnalogKey<A2, 3> keys;

MicroDS18B20 <14> DS;  // датчик на A0 НЕ ЗАБЫВАЕМ ПОДКЛЮЧИТЬ PULL-UP РЕЗИСТОР!!!
MicroDS3231 rtc;
GyverBME280 bme;
AHT20 aht20;
//GyverHTU21D htu;
...
#define INIT_KEY 102     // ключ первого запуска. 0-254, на выбор. Сначало прошить с ключём, а потом изменить ключ и прошить.
...
void setup() 
{
  DDRB = 0xEF; // Конфигурируем PortB
  PORTB = 0b00010000;
  // PB0 (D8) - SIN (выход)
  // PB1 (D9) - CLK (выход)
  // PB2 (D10) - LATCH (выход)
  // PB3 (D11) - BLK (выход)
  // PB4 (D12) - SO (вход с подтяжкой к плюсу)

  pinMode(3, OUTPUT); // Зуммер на D3
  // pinMode(5, INPUT_PULLUP); // Датчик положения на D5
  enc.pullUp(); // Включить подтягивающие резисторы
  // enc.setEncType(EB_FULLSTEP);  // тип энкодера: EB_FULLSTEP (0) по умолч., EB_HALFSTEP (1) если энкодер делает один поворот за два щелчка

  // Disp_detect();  // Определяем тип дислея
 
  // Настройка таймера 1 на прерывание
  cli();
  TCCR1A = 0b00000000;    // OC1A Отключен
  TCCR1B = 0b00001011;    // делитель 64, сброс при совпадении 1A (режим СТС)
//  OCR1A = 0x007D;         // 100 Гц частота сканирования сеток дисплея
  OCR1A = 125;         // 100 Гц частота сканирования сеток дисплея
  TIMSK1 = 0b0000010;     // запуск прерывания А таймера 1
  sei();
...
//  readDS18B20();           // считываем температуру
  ...
  /*
    if (htu.begin())  {   //_____Определяем подключен ли датчик HTU21D
      htu_online = 1;  //___HTU21D подключен
    } else  {
      htu_online = 0;   //___HTU21D не подключен
    }
    delay (100);
    if (htu_online) {
      get_temp_indoor();
  ...
  */
  //Радостно пищим 3 раза и переходим к выполнению основного кода
  // beep();
  // delay(1);
  // beep();
  // delay(1);
  // beep();
  tone(piezoPin, 10400, 500);
}
//---------------------------------------------------------------------------------------------------------
void loop() 
{
  ...
  if (millis() - light_timer >= 1000) 
      { //_______Измеряем освещённость и температуру раз в секунду
        ...
        light = analogRead(20) >> 2; //___Датчик света подключен к 20 пину (А6). Используем старшие 8 бит АЦП

...
bool MicroDS3231::begin(void){
	Wirer.begin();                       // Инит шины
	Wirer.beginTransmission(_addr);      // Зовем DS3231 по адресу
    return (!Wirer.endTransmission());   // если никто не откликнулся - возвращаем false
}
...
void MicroDS3231::setTime(int8_t seconds, int8_t minutes, int8_t hours, int8_t date, int8_t month, int16_t year) {
    ...
    year -= 2000;
    Wirer.beginTransmission(_addr);
    Wirer.write(0x00);
    Wirer.write(encodeRegister(seconds));
    Wirer.write(encodeRegister(minutes));
    if (hours > 19) Wirer.write((0x2 << 4) | (hours % 20));
    else if (hours > 9) Wirer.write((0x1 << 4) | (hours % 10));
    else Wirer.write(hours);	
    Wirer.write(day);
    Wirer.write(encodeRegister(date));
    Wirer.write(encodeRegister(month));
    Wirer.write(encodeRegister(year));
...
DateTime MicroDS3231::getTime() {
    ...
    Wirer.beginTransmission(_addr);
    Wirer.write(0x0);
    if (Wirer.endTransmission() != 0) return now;
    Wirer.requestFrom(_addr, (uint8_t)7);
    now.second = unpackRegister(Wirer.read());
    now.minute = unpackRegister(Wirer.read());
    now.hour = unpackHours(Wirer.read());
    now.day = Wirer.read();
    now.date = unpackRegister(Wirer.read());
    now.month = unpackRegister(Wirer.read());
    now.year = unpackRegister(Wirer.read()) + 2000;
    return now;
}
...
uint8_t MicroDS3231::readRegister(uint8_t addr) {
    ...
    Wirer.requestFrom(_addr, (uint8_t)1);
    uint8_t data = Wirer.read();
    return data;
}
...
int MicroDS3231::getTemperatureRaw(void) {
    ...
    Wirer.write(0x11);
    Wirer.endTransmission();
    Wirer.requestFrom(_addr, (uint8_t)2);
    return ((int8_t)Wirer.read() << 2) + (Wirer.read() >> 6);
}
...
//        shiftOut(DIN_PIN, CLK_PIN, MSBFIRST, CMD_DIGIT7 - c);
//        shiftOut(DIN_PIN, CLK_PIN, MSBFIRST, bt);
    }
    digitalWrite(CS_PIN, HIGH);
    mask<<=1;
...
void scrollLeft()
{
  for(byte i=0; i < NUM_MAX*8+7; i++) scr[i] = scr[i+1];
}

void invert()
{
  for (byte i = 0; i < NUM_MAX*8; i++) scr[i] = ~scr[i];
}
...
void initMAX7219()
{
  ...
  sendCmdAll(CMD_INTENSITY, 7); // minimum brightness

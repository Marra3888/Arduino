...
unsigned char z_PosX = 0;                            
unsigned char d_PosX = 0;                            
bool f_tckr1s = false;
bool f_tckr50ms = false;
bool f_tckr500ms = false;
...
struct MyDateTime 
{
    unsigned char sek1, sek2, sek12, min1, min2, min12, hour1, hour2, hour12;
    unsigned char tag1, tag2, tag12, mon1, mon2, mon12, year1, year2, year12, WT;
} MEZ;
...
unsigned char const font3[27][9] =
{
...
void Read__Time()
{
    // DateTime RTC = rtc.now(); // Читаем время из RTC
    ...
    // Tag = now.day(); // Получаем локальный день
    // Monat = now.month(); // Получаем локальный месяц
    // Jahr = now.year(); // Получаем локальный год
    // WoTag = now.dayOfTheWeek(); // weekday() возвращает 1-7 (Вс-Сб), корректируем на 0-6
    // Stunde = now.hour(); // Получаем локальные часы
    // Minute = now.minute(); // Получаем локальные минуты
    // Sekunde = now.second(); // Получаем локальные секунды
    // Serial.println(Tag);
    ...
    if (RTC.readTime()) 
        {
            Tag = RTC.dd; // Получаем локальный день
            Monat = RTC.mm; // Получаем локальный месяц
            Jahr = RTC.yyyy; // Получаем локальный год
            WoTag = RTC.dow; // weekday() возвращает 1-7 (Вс-Сб), корректируем на 0-6
            Stunde = RTC.h; // Получаем локальные часы
            Minute = RTC.m; // Получаем локальные минуты
            Sekunde = RTC.s; // Получаем локальные секунды
        }
    else 
        {
                Tag = 1; // Получаем локальный день
                Monat = 1; // Получаем локальный месяц
                Jahr = 25; // Получаем локальный год
                WoTag = 1; // weekday() возвращает 1-7 (Вс-Сб), корректируем на 0-6
                Stunde = 20; // Получаем локальные часы
                Minute = 10; // Получаем локальные минуты
                Sekunde = 0; // Получаем локальные секунды     
        }
    // Serial.println(Sekunde);
    // Serial.println(WoTag);
...
//                         Tag = ti->tm_mday;
//                         Monat = ti->tm_mon + 1;    // Месяцы начинаются с 0, добавляем 1
//                         Jahr = ti->tm_year + 1900; // Годы отсчитываются от 1900
//                         WoTag = ti->tm_wday;    // День недели (0 - воскресенье, 6 - суббота)
//                         Stunde = ti->tm_hour;
//                         Minute = ti->tm_min;
//                         Sekunde = ti->tm_sec;
...
void adjust_time_date() 
{
    ...
    if (timeClient.update()) 
      { 
            // Обновляем время с NTP
        // unsigned long epochTime = timeClient.getEpochTime();//timeClient.getEpochTime() возвращает время в формате Unix (секунды с 1 января 1970 года).
        //Получаем Epoch Time в UTC
        unsigned long epochTimeUTC = timeClient.getEpochTime();
        ...
        // Преобразуем время Unix в компоненты
        // time_t rawtime = epochTime;
        // struct tm * ti = localtime(&rawtime);//Функция localtime из стандартной библиотеки C (<time.h>) преобразует время Unix в структуру tm, содержащую компоненты времени.

        // Преобразование локального времени в структуру tm
        struct tm *ti;
        ti = localtime(&localTime);

        RTC.yyyy = ti->tm_year + 1900;//Значения из структуры tm присваиваются переменным объекта RTC, 
        RTC.mm = ti->tm_mon + 1;//после чего вызывается RTC.writeTime() для записи времени в RTC (см. MD_DS3231::writeTime() в MD_DS3231.cpp).
        RTC.dd = ti->tm_mday;
        RTC.h = ti->tm_hour;
        RTC.m = ti->tm_min;
        RTC.s = ti->tm_sec;
        RTC.dow = ti->tm_wday + 1;

        RTC.writeTime(); // Записываем время в RTC
      }
    // Serial.println("RTC time adjusted via NTP");
    // Serial.print("NTP Epoch Time: "); Serial.println(epochTime);
    ...
    // Serial.print(RTC.s); Serial.print(", dow=");
...
void timer50ms() 
{
    ...
    if (cnt50ms == 20) 
    {
        ...
        cnt50ms = 0;
            ...
            {
                f_tckr500ms = !f_tckr500ms; // 0.5 sec
                // cnt50ms = 0;
            }
}
//**************************************************************************************************
...
{ //символы в arr
    ...
    PosX++;
    k = ch - 32; // Позиция ASCII в шрифте
    if ((fontType == FONT3 && k >= 0 && k < 32) || ((fontType == FONT1 || fontType == FONT2) && k >= 0 && k < 96)) 
        { // Символ найден в шрифте?
...
//-------------------------------------------------------------------------------------------------------------------
    ...
    //     char22Arr('N', 6, 0);
        ...
        // rtc.adjust(DateTime(2025, 9, 11, 20, 50, 0));
    //     refresh_display();
    // }
    // else 
    // {
...
//Бесконечный цикл    
    while (1) 
    {
        //   ESP.wdtFeed();
        //   timeClient.update();
        // ESP.wdtDisable(); // Отключает оба WDT (осторожно, может привести к нестабильности)
          if (jj++ > 3000)
            {
                jj = 0;
                // readGPS();
                    ...
                    if (nomber1 == 1)//--------------------------- nomber 1 ------------------------------- 
                        {
                            ...
                            char2Arr(48 + sek12, z_PosX - sec1_position, y3, FONT2);  //31 - 27 = 4
                            char2Arr(48 + sek11, z_PosX - sec1_position, y + y_1, FONT2);  //31 - 27 = 4
                        ...
                        char2Arr(48 + sek1, z_PosX - sec1_position, 0, FONT2);//31 -  27 = 4


                    if (nomber2 == 1)//-------------------------- nomber 2 ------------------------------------------ 
                        {

                            char2Arr(48 + sek22, z_PosX - sec2_position, y3, FONT2);//31 - 23 = 8
                            char2Arr(48 + sek21, z_PosX - sec2_position, y + y_1, FONT2);//31 - 23 = 8

                            if (y == 0)   nomber2 = 0;
                        }//-------------------------------------- end nomber 2 --------------------------------------
                    else
                        char2Arr(48 + sek2, z_PosX - sec2_position, 0, FONT2);//31 - 23 = 8


                    if (nomber3 == 1)//-------------------------- nomber 3 ------------------------------------------ 
                        {

                            char2Arr(48 + min12, z_PosX - min1_position, y, FONT1);//31 - 19 = 12
                            char2Arr(48 + min11, z_PosX - min1_position, y + y_1, FONT1);//31 - 19 = 12

                            if (y == 0)    nomber3 = 0;
                        }//-------------------------------------- end nomber 3 --------------------------------------
                    else
                        char2Arr(48 + min1, z_PosX - min1_position, 0, FONT1);//31 - 19 = 12


                    if (nomber4 == 1)//-------------------------- nomber 4 ------------------------------------------ 
                        {

                            char2Arr(48 + min22, z_PosX - min2_position, y, FONT1);//31 - 14 = 17
                            char2Arr(48 + min21, z_PosX - min2_position, y + y_1, FONT1);//31 - 14 = 17

                            if (y == 0)   nomber4 = 0;
                        }//-------------------------------------- end nomber 4 --------------------------------------
                    else

                        char2Arr(48 + min2, z_PosX - min2_position, 0, FONT1);//31 - 14 = 17
    //                if (jg++ > 3000)
    //                  {
    //                    jg = 0;
                        char2Arr(':', z_PosX - colon_position + x, 0, FONT1);//31 - 11 = 20
    //                  }

                    if (nomber5 == 1)//-------------------------- nomber 5 ------------------------------------------ 
                        {

                            char2Arr(48 + hour12, z_PosX - hour1_position, y, FONT1);//31 - 4 = 27
                            char2Arr(48 + hour11, z_PosX - hour1_position, y + y_1, FONT1);//31 - 4 = 27

                            if (y == 0)   nomber5 = 0;
                        }//-------------------------------------- end nomber 5 --------------------------------------
                    else
                        char2Arr(48 + hour1, z_PosX - hour1_position, 0, FONT1);//31 - 4 = 27


                    if (nomber6 == 1)//-------------------------- nomber 6 ------------------------------------------ 
                        {

                            char2Arr(48 + hour22, z_PosX + hour2_position, y, FONT1);//31 + 1 = 32
                            char2Arr(48 + hour21, z_PosX + hour2_position, y + y_1, FONT1);//31 + 1 = 32

                            if (y == 0)   nomber6 = 0;
                        }//-------------------------------------- end nomber 6 --------------------------------------
                    else
                        char2Arr(48 + hour2, z_PosX + hour2_position, 0, FONT1);//31 + 1 = 32


                        char2Arr(' ', d_PosX + 5, 0, FONT1);        //day of the week
                }
...
//*************************************************************************************************************************************************************
            ...
            // #ifdef without_seconds
            else 
                {
                    if (nomber3 == 1)//-------------------------- nomber 3 ------------------------------------------ 
                        {
                            char2Arr(48 + min12, z_PosX - min1_position - 8, y, FONT3);//31 - 19 = 12
                            char2Arr(48 + min11, z_PosX - min1_position - 8, y + y_1, FONT3);//31 - 19 = 12

                            if (y == 0)   nomber3 = 0;
                        }//-------------------------------------- end nomber 3 --------------------------------------
                    else
                        char2Arr(48 + min1, z_PosX - min1_position - 8, 0, FONT3);//31 - 19 = 12


                    if (nomber4 == 1)//-------------------------- nomber 4 ------------------------------------------ 
                        {
                            char2Arr(48 + min22, z_PosX - min2_position - 6, y, FONT3);//31 - 14 = 17
                            char2Arr(48 + min21, z_PosX - min2_position - 6, y + y_1, FONT3);//31 - 14 = 17

                            if (y == 0)   nomber4 = 0;
                        }//-------------------------------------- end nomber 4 --------------------------------------
                    else
                        char2Arr(48 + min2, z_PosX - min2_position - 6, 0, FONT3);//31 - 14 = 17

                    if (f_tckr500ms)//-------------------------- ":"-------------------------------------------------
                                    char2Arr(':', z_PosX - colon_position - 5, 0, FONT3);//31 - 11 = 20
                    else 
                        char2Arr(' ', z_PosX - colon_position - 5, 0, FONT3);//31 - 11 = 20
                       
                    if (nomber5 == 1)//-------------------------- nomber 5 ------------------------------------------ 
                        {
                            char2Arr(48 + hour12, z_PosX - hour1_position - 4, y, FONT3);//31 - 4 = 27
                            char2Arr(48 + hour11, z_PosX - hour1_position - 4, y + y_1, FONT3);//31 - 4 = 27

                            if (y == 0)   nomber5 = 0;
                        }//-------------------------------------- end nomber 5 --------------------------------------
                    else
                        char2Arr(48 + hour1, z_PosX - hour1_position - 4, 0, FONT3);//31 - 4 = 27


                    if (nomber6 == 1)//-------------------------- nomber 6 ------------------------------------------ 
                        {
                            char2Arr(48 + hour22, z_PosX + hour2_position - 2, y, FONT3);//31 + 1 = 32
                            char2Arr(48 + hour21, z_PosX + hour2_position - 2, y + y_1, FONT3);//31 + 1 = 32

                            if (y == 0)   nomber6 = 0;
                        }//-------------------------------------- end nomber 6 --------------------------------------
                    else
                        char2Arr(48 + hour2, z_PosX + hour2_position - 2, 0, FONT3);//31 + 1 = 32
...
//            char2Arr(szMesg[3], d_PosX - 19, 0);
            ...
            len += 7;
            char2Arr(48 + MEZ.tag2, d_PosX - len, 0, FONT1); //56          //day
            len += 6;
            char2Arr(48 + MEZ.tag1, d_PosX - len, 0, FONT1);//62
            len += 6;
            char2Arr(' ', d_PosX + len, 0, FONT1);
            len += 3;
...
//            char2ArrRus(M_rus[MEZ.mon12 - 1][4], d_PosX - len, 0);
            len += 6;
            char2Arr('2', d_PosX - len, 0, FONT1);//142                     //year
            len += 6;
            char2Arr('0', d_PosX - len, 0, FONT1);//148
            len += 6;
            char2Arr(48 + MEZ.year2, d_PosX - len, 0, FONT1);//154
            len += 6;
            char2Arr(48 + MEZ.year1, d_PosX - len, 0, FONT1);//160
            len += 6;
            char2Arr(' ', d_PosX - len, 0, FONT1);//167
            #else
            char2Arr(WT_arr[MEZ.WT][0], d_PosX - 1, 0, FONT1);        //day of the week
            char2Arr(WT_arr[MEZ.WT][1], d_PosX - 7, 0, FONT1);
            char2Arr(WT_arr[MEZ.WT][2], d_PosX - 13, 0, FONT1);
            char2Arr(WT_arr[MEZ.WT][3], d_PosX - 19, 0, FONT1);
            char2Arr(48 + MEZ.tag2, d_PosX - 26, 0, FONT1);           //day
            char2Arr(48 + MEZ.tag1, d_PosX - 32, 0, FONT1);
            char2Arr(M_arr[MEZ.mon12 - 1][0], d_PosX - 39, 0, FONT1); //month
            char2Arr(M_arr[MEZ.mon12 - 1][1], d_PosX - 43, 0, FONT1);
            char2Arr(M_arr[MEZ.mon12 - 1][2], d_PosX - 49, 0, FONT1);
            char2Arr(M_arr[MEZ.mon12 - 1][3], d_PosX - 55, 0, FONT1);
            char2Arr(M_arr[MEZ.mon12 - 1][4], d_PosX - 61, 0, FONT1);
            char2Arr('2', d_PosX - 68, 0, FONT1);                     //year
            char2Arr('0', d_PosX - 74, 0, FONT1);
            char2Arr(48 + MEZ.year2, d_PosX - 80, 0, FONT1);
            char2Arr(48 + MEZ.year1, d_PosX - 86, 0, FONT1);
            char2Arr(' ', d_PosX - 93, 0, FONT1); 
            #endif
            len = 0;

...
public:
  ...
  // Очистка D‑данных
  void clear() {
    memset(Display_Data_0, 0, 13);  // D1..D108 (0..13)
...
void runFullLcdTest() {
  ...
  lcd.testSegmentsOnLogical(0, 400);
...
void setup() {
  ...
  // 1) Все сегменты в 0‑й логической (крайняя левая)
  lcd.testLogicalCell(0);
  delay(2000);

  // 2) Все сегменты в 1‑й логической
  lcd.clear();
  lcd.testLogicalCell(1);
  delay(2000);

  // 3) Попробуем цифры:
  lcd.clear();
  lcd.putChar(0, '8'); // '8' у всех сегментов
  lcd.putChar(1, '8');
  lcd.display();

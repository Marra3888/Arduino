...
#include <Timezone.h>
#include <Time.h>
// Добавляем библиотеку для DS3231
...
int Jahr;
// volatile byte Tag, Monat, WoTag, Stunde, Minute, Sekunde;
// volatile int Jahr;
...
//        Sekunde = 0;
    ...
    // time_t localTime = myTZ.toLocal(now.unixtime());

    // Jahr = year(localTime);
    // if (Jahr > 99) Jahr = 0; // Ограничиваем год до двух цифр
    // Monat = month(localTime);
    // if (Monat > 12) Monat = 0;
    // Tag = day(localTime);
    // if (Tag > 31) Tag = 0;
    // WoTag = weekday(localTime) - 1; // День недели (0 = воскресенье)
    // if (WoTag == 7) WoTag = 0;
    // Stunde = hour(localTime);
    // if (Stunde > 23) Stunde = 0;
    // Minute = minute(localTime);
    // if (Minute > 59) Minute = 0;
    // Sekunde = second(localTime);
...
// void char2ArrRus(uint8_t ch, char PosX, short PosY) 
// { //символы в arr с вращением вокруг своей оси
//     uint8_t i, j, k, l, m, o1, o2, o3, o4;  //в LEDarr
//     PosX++;
//     if (ch == 0xff)  return;
...
//         for (i = 0; i < o4; i++) 
//         {
//             if ((PosX - i <= maxPosX) && (PosX - i >= 0)) //проверка горизонтальных границ матрицы
//             {
//                 o1 = helpArrPos[PosX - i];
//                 o2 = helpArrMAX[PosX - i];
...
//                 for (j = 0; j < 8; j++) 
//                 {
//                     // Циклическая прокрутка по вертикали вокруг своей оси
//                     uint8_t rotatedY = (j + PosY) % 8; //остаток от деления для циклического вращения
//                     if (rotatedY < 0) rotatedY += 8;   //обработка отрицательных значений
                    
//                     l = Rus[k][j + 1];
//                     m = (l & (o3 >> i));
//                     if (m > 0)
//                         LEDarr[o2][rotatedY] = LEDarr[o2][rotatedY] | (o1);  //установка точки
//                     else
//                         LEDarr[o2][rotatedY] = LEDarr[o2][rotatedY] & (~o1); //очистка точки
//                 }
//             }
//         }
//     }
...
void timer50ms() 
{
    ...
    {
        f_tckr1s = true; // 1 sec
        // readRTC();  // Вызываем здесь, а не в loop()
        cnt50ms = 0;
    }
}
//**************************************************************************************************
...
//Бесконечный цикл    
    while (true) 
    {
          ...
          if (jj++ > 3000)
          {
            ...
            // readGPS();
            readRTC();  // Заменяем readGPS на readRTC
            Serial.println("");
            // delay(90);
            jj = 0;
          }
...
//     return (Wire.endTransmission() == 0); // 0 = успех
// }
// Заменяем функцию readGPS
void readRTC()
{
  // if (isRTCReady()) 
  // {
    ...
    // WoTag = weekday(localTime) - 1;

                  // DateTime now = RTC.now();
                  Jahr = now.year();       // присваиваем переменным значения года
                  Monat = now.month();   // месяца
                  Tag = now.day();      // дня
                  WoTag = now.dayOfTheWeek() + 1;
                  // day_week = 6;
                  Stunde = now.hour();      // часа
                  Minute = now.minute();  // минуты
                  Sekunde = now.second(); // и секунды
// }
}
